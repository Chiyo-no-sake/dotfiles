...
BarWidget {
  ...
  // UseQApplication` — this shell does not, so every submenu click was
  ...
  // Tray items whose app already has a first-party bar widget never also
  // show as icons — the widget is the surface, the icon would be an echo.
  // Omarchy hid LocalSend and Dropbox this way; this shell ships no such
  // widget yet, so the list is empty and every icon shows. Fill it with app
  // name fragments (matched case-insensitively against id/title/tooltip)
  // when a module takes over an app.
  readonly property var ownedApps: []

  function ownedByWidget(item) {
    return TrayModel.ownedByWidget(item, ownedApps)
  }
  ...
  function bucket(category) {
    ...
    for (var i = 0; i < values.length; i++) {
      ...
      if (ownedByWidget(item)) continue
      if (category === "all") {
        result.push(item)
        continue
      }
  ...
  function persistTrayState(pinned, hidden) {
    ...
    // Entry shape updateEntryInline expects: the bare layout id ("tray") —
    // the shell strips the "qs." prefix from the moduleName argument and
    // rebuilds the entry id itself — plus the inline settings.
    var entry = { id: String(root.moduleName || "qs.tray").replace(/^qs\./, ""), pinned: pinned, hidden: hidden }
    // Applied locally first: the host only injects settings when a widget
    // loads, so the layout.json write alone would leave this instance's
    // pinned/hidden stale until the next reload.
    root.settings = entry
    root.bar.shell.updateEntryInline(root.moduleName || "qs.tray", entry)
  }
