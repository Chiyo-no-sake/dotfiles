...
QtObject {
  ...
  // Follows the qs-display-text-size state file live: the Display panel's
  // text-size slider writes it, this watch reflows the whole shell.
  property int fontBaseSize: 12

  readonly property string textSizeStatePath: Quickshell.env("HOME") + "/.local/state/qs/display-text-size"

  function applyTextSize(raw) {
    ...
    if (isFinite(n) && n >= 1 && n <= 72) fontBaseSize = n
  }

  property FileView textSizeStateFile: FileView {
    path: root.textSizeStatePath
    watchChanges: true
    printErrors: false
    onLoaded: root.applyTextSize(text())
    onLoadFailed: root.applyTextSize("")
  ...
  readonly property QtObject bar: QtObject {
    ...
    readonly property int sizeVertical:   root.barToken("size-vertical",   34)
    readonly property int iconSlot:       root.barToken("icon-slot",       33)
    readonly property int iconCanvas:     root.barToken("icon-canvas",     19)
    readonly property int iconFont:       root.barToken("icon-font",       15)
    readonly property int statusSlot:     root.barToken("status-slot",     25)
  }
