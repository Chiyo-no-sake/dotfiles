...
Panel {
  ...
  function summonWifiQr(forceDetect) {
    ...
    // shell.summon returns false when no overlay plugin is registered (v1
    // has none) — fire-and-forget, so the panel just closes and the card
    // stays absent until one is.
    if (bar && bar.shell) bar.shell.summon("qs.wifiqr", JSON.stringify(payload))
  }
  ...
  function refresh(scanWifi) {
    ...
    if (!bandProc.running) {
      bandProc.command = ["qs-network-band"]
  ...
  function setBand(band) {
    ...
    actionProc.command = ["qs-network-band", band]
  ...
  function summonSpeedTest() {
    ...
    // False return (no overlay registered yet) is fine: fire-and-forget, and
    // the panel is already closed at this point.
    if (bar && bar.shell) bar.shell.summon("qs.speedtest", connection ? JSON.stringify({ connection: connection }) : "{}")
  }
  ...
  function setDns(provider) {
    ...
    if (provider === "Custom") {
      var launcher = "qs-launch-floating-terminal-with-presentation"
