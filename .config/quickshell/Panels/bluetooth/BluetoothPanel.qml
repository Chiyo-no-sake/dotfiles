...
Panel {
  ...
  // The actual Bluetooth sequencing lives in bin/qs-bluetooth-device;
  ...
  function deviceCommand(action, address) {
    return ["qs-bluetooth-device", action, address]
  }
  ...
  // disconnects, then removes the BlueZ pairing record via qs-bluetooth-device.
  ...
  // the adapter came back on at the next boot. qs-bluetooth-power moves the
  ...
  function toggleBluetooth() {
    if (!adapter) return
    Quickshell.execDetached(["qs-bluetooth-power", adapter.enabled ? "off" : "on"])
  }
