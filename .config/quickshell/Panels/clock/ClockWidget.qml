...
// middle click does nothing (the timezone picker it used to launch was an
// omarchy script that was not ported).
BarWidget {
  id: root
  moduleName: "qs.clock"

  // The host grants this to exactly one instance per moduleName (the others
  // relay through broadcast), and only the owner may register the IPC target.
  ...
  // What the bar shows is what layout.json stores, so a cycled format is the
  ...
  function cycleFormat() {
    ...
    // Entry shape updateEntryInline expects: the module's bare layout id
    // ("clock" — the shell strips the "qs." prefix from the moduleName
    // argument and rebuilds the entry id itself) plus the inline settings.
    var entry = { id: String(root.moduleName).replace(/^qs\./, "") }
    for (var key in root.settings) if (key !== "id") entry[key] = root.settings[key]
    entry[vertical ? "verticalFormat" : "format"] = next

    // Applied locally first so the label changes on the click itself; the
    // layout.json write comes back through the bar as the same value.
    root.settings = entry
  ...
  IpcHandler {
    // Only the instance the shell granted IPC ownership to may register
    // the target; a duplicate registration on the next monitor would fail.
  ...
  WidgetButton {
    ...
    onPressed: function(b) {
      ...
      else if (b === Qt.MiddleButton) { /* no-op: timezone picker not ported */ }
