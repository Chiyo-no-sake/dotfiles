...
//
// ClockWidget.qml owns the bar label and hands this panel the button to
// anchor against.
Panel {
  id: root
  moduleName: "qs.clock"
  // The host widget (ClockWidget.qml) owns the IPC target; this nested
  // panel's own handler stays disabled to avoid a duplicate registration.
  ipcTarget: "qs.clock"
  manageIpc: false

  property var anchorItem: null

  // The bar tracks the widget mounted in its slot — ClockWidget.qml — not
  // this nested panel. Everything the bar identifies a panel by has to be
  // that widget: the popout coordinator (and with it the open-panel dot
  // under the pill) compares against `slot.activeItem`, and
  ...
  // layout.json write comes back through the bar as the same value. With no
  ...
  function persistSettings(values) {
    // Same entry shape the host widget uses: the bare layout id ("clock")
    // plus the inline settings — updateEntryInline rebuilds the entry id
    // from the moduleName argument anyway.
