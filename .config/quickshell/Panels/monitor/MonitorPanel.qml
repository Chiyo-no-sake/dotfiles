...
Panel {
  ...
  // ---- theme (light/dark via matugen engine) ----
  readonly property string home: Quickshell.env("HOME") || ""
  readonly property string themeModeFile: home + "/.cache/theme-mode"
  property string themeMode: "dark"

  function applyTheme(mode) {
    if (mode === themeMode || themeProc.running) return
    // pty wrapper: matugen needs a controlling terminal (see script header)
    themeProc.command = ["bash", "-c", "'" + home + "/.local/share/bin/qs-theme-toggle' " + mode]
  ...
  function setBrightness(value) {
    ...
    setBrightnessProc.command = ["qs-brightness-display", "--no-osd", "--monitor", root.focusedMonitor, percent + "%"]
  ...
  function setScale(scale) {
    actionProc.command = ["bash", "-c", "qs-hyprland-monitor-scaling " + scale]
    if (!actionProc.running) actionProc.running = true
  }
  ...
  function setTextSize(px) {
    textScaleProc.command = ["qs-display-text-size", String(px)]
  ...
  property FileView themeModeView: FileView {
    path: root.themeModeFile
    watchChanges: true
    printErrors: false
    onLoaded: root.applyThemeModeText(text())
    onLoadFailed: root.applyThemeModeText("")
    onFileChanged: reload()
  }

  function applyThemeModeText(raw) {
  ...
  Process {
    ...
    // The engine regenerates every theme output including our colors.json;
    // the Color singleton's watch handles the re-theme. Nothing to do here
    // beyond letting the exit state settle.
  }
    ...
    PanelKeyCatcher {
      ...
      ScrollView {
        ...
        Column {
          ...
          // ---------- Theme ----------
          // Light/dark via the user's theme-toggle engine: matugen reruns on
          // the current wallpaper for the chosen mode and every themed app
          // reloads — including this shell, whose Color singleton watches
          // the generated colors.json.
          PanelSeparator {
            foreground: root.bar.foreground
          }
            ...
            Grid {
              ...
              Repeater {
                ...
                Button {
                  ...
                  bordered: true
                  active: root.themeMode === modelData.id
                  tooltipText: "Switch to " + modelData.label.toLowerCase() + " theme"
